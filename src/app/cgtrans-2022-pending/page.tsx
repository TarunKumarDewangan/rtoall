...
import { supabase, CGTrans2022PendingRow, fetchAllRows } from '@/lib/supabase'
...
// fixed schema — whatever headers show up become that row's JSON keys.
// This page holds the older 2022 CGTrans pending-data batch specifically,
// kept separate from EV Final V1 so the two datasets never mix.
...
// of breaking it.
function computeColumns(rows: CGTrans2022PendingRow[]): string[] {
...
const VISIBLE_COLS_STORAGE_KEY = 'cgtrans_2022_pending_visible_columns'

export default function CGTrans2022PendingPage() {
  const [input, setInput] = useState('')
  const [saving, setSaving] = useState(false)
  const [saveProgress, setSaveProgress] = useState<{ done: number; total: number } | null>(null)
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null)

  const [savedRows, setSavedRows] = useState<CGTrans2022PendingRow[]>([])
  ...
  const [editRow, setEditRow] = useState<CGTrans2022PendingRow | null>(null)
  const [editForm, setEditForm] = useState<Record<string, string>>({})
  const [savingEdit, setSavingEdit] = useState(false)

  const [pinAction, setPinAction] = useState<null
    | { type: 'edit'; row: CGTrans2022PendingRow }
  ...
  async function fetchSaved() {
    setLoading(true)
    const { data, error } = await fetchAllRows<CGTrans2022PendingRow>('cgtrans_2022_pending', 'created_at', false)
  ...
  async function handleSave() {
    ...
    for (let i = 0; i < payload.length; i += SAVE_CHUNK_SIZE) {
      ...
      const { error } = await supabase.from('cgtrans_2022_pending').insert(chunk)
      if (error) {
        setSaving(false)
  ...
  // the 4-digit years found inside them.
  ...
  // "identity" column — since this table has no fixed schema, the user
  // picks which column actually identifies a unique record.
  ...
  const duplicateIdsToRemove = useMemo(() => {
    if (!dupColumn) return []
    const groups: Record<string, CGTrans2022PendingRow[]> = {}
    savedRows.forEach(r => {
      const v = r.row_data?.[dupColumn]
      if (!v) return
      ;(groups[v] ||= []).push(r)
  ...
  function requestEdit(row: CGTrans2022PendingRow) { setPinAction({ type: 'edit', row }) }
  ...
  function openEdit(row: CGTrans2022PendingRow) {
    setEditRow(row)
  ...
  async function handleSaveEdit() {
    ...
    setSavingEdit(true)
    const { error } = await supabase.from('cgtrans_2022_pending').update({ row_data: editForm }).eq('id', editRow.id)
    setSavingEdit(false)
    if (error) showMsg('error', error.message)
    else {
  ...
  async function handleRemoveDuplicates() {
    ...
    for (let i = 0; i < duplicateIdsToRemove.length; i += CHUNK) {
      ...
      const { error } = await supabase.from('cgtrans_2022_pending').delete().in('id', chunk)
      if (error) {
    ...
    const { data: freshData, error: refetchError } = await fetchAllRows<CGTrans2022PendingRow>('cgtrans_2022_pending', 'created_at', false)
    setRemovingDuplicates(false)
    setShowRemoveDuplicatesConfirm(false)
  ...
  async function handleDelete(id: number) {
    const { error } = await supabase.from('cgtrans_2022_pending').delete().eq('id', id)
    setDeleteId(null)
    if (error) showMsg('error', error.message)
  ...
  async function handleDeleteAll() {
    setDeletingAll(true)
    const { error } = await supabase.from('cgtrans_2022_pending').delete().neq('id', 0)
  ...
  function handleExportExcel() {
    ...
    exportToExcel(`cgtrans_2022_pending_${new Date().toISOString().slice(0, 10)}.xlsx`, visibleColumns, rows)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="w-full px-4 py-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-blue-900">CGTrans 2022 Pending Data</h1>
          <p className="text-sm text-gray-500 mt-0.5">पहली पंक्ति हेडर होनी चाहिए — Excel से पूरी टेबल (हेडर सहित) कॉपी-पेस्ट करें, कोई भी कॉलम-सेट चलेगा। यह डेटा EV Final V1 से अलग यहीं रहता है।</p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 mb-6">
          ...
          <textarea
            ...
            rows={8}
            placeholder="SNo	VehicleNo	OwnerName	GuardianName	MobileNo	...  ← पहली पंक्ति हेडर होनी चाहिए"
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <select
            ...
            className="border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
          >
            <option value="">-- कॉलम चुनें (जैसे VehicleNo / ChasisNo) --</option>
            {columns.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
          {dupColumn && (
            <>
