...
export default function GhoshnapatraPage() {
  const [entries, setEntries] = useState<GhosnapatraEntry[]>([])
  const [filtered, setFiltered] = useState<GhosnapatraEntry[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')
  const [showForm, setShowForm] = useState(false)
  const [editEntry, setEditEntry] = useState<GhosnapatraEntry | null>(null)
  const [form, setForm] = useState<FormType>(EMPTY_FORM)
  const [saving, setSaving] = useState(false)
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null)
  const [deleteId, setDeleteId] = useState<number | null>(null)
  const [pinAction, setPinAction] = useState<null | { type: 'edit'; entry: GhosnapatraEntry } | { type: 'delete'; id: number }>(null)
  ...
  useEffect(() => {
    ...
    setFiltered(entries.filter((e: any) =>
      ...
      e.owner_name?.toLowerCase().includes(s)
    ))
  }, [entries, search])
  ...
  function requestEdit(entry: GhosnapatraEntry) { setPinAction({ type: 'edit', entry }) }
  ...
  function openEdit(entry: GhosnapatraEntry) {
    setEditEntry(entry)
    setForm({
      vehicle_no: (entry as any).vehicle_no || '',
      engine_no: (entry as any).engine_no || '',
      chassis_no: (entry as any).chassis_no || '',
      model: (entry as any).model || '',
      owner_name: (entry as any).owner_name || '',
      father_name: (entry as any).father_name || '',
      verify_date: (entry as any).verify_date || '',
      verifier: (entry as any).verifier || '',
      designation: (entry as any).designation || '',
    })
    setShowForm(true)
  }
  ...
  async function handleSave() {
    setSaving(true)
    const payload = { ...form, verify_date: form.verify_date || null }
    let error: any
    if (editEntry && (editEntry as any).id) {
      ;({ error } = await supabase.from('ghosnapatra_entries').update(payload).eq('id', (editEntry as any).id))
    } else {
      ;({ error } = await supabase.from('ghosnapatra_entries').insert(payload))
    }
    setSaving(false)
    if (error) showMsg('error', error.message)
    else {
  ...
  async function handleDelete(id: number) {
    const { error } = await supabase.from('ghosnapatra_entries').delete().eq('id', id)
    setDeleteId(null)
    if (error) showMsg('error', error.message)
    else { showMsg('success', 'Entry deleted.'); fetchEntries() }
  }

  const formFields: { label: string; key: keyof FormType; type: string }[] = [
    ...
    { label: 'Verify Date', key: 'verify_date', type: 'date' },
    { label: 'Verifier', key: 'verifier', type: 'text' },
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-blue-900">घोषणापत्र (Ghoshnapatra)</h1>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="bg-blue-900 text-white">
                  {['#', 'Vehicle No', 'Engine No', 'Chassis No', 'Model', 'Owner Name', 'Father Name', 'Verify Date', 'Verifier', 'Designation', 'Actions'].map(h => (
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={entry.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-2 text-xs font-mono">{entry.engine_no}</td>
                    <td className="px-3 py-2 text-xs font-mono">{entry.chassis_no}</td>
                    <td className="px-3 py-2 text-xs">{entry.model}</td>
                    <td className="px-3 py-2 text-xs font-medium">{entry.owner_name}</td>
                    <td className="px-3 py-2 text-xs">{entry.father_name}</td>
                    <td className="px-3 py-2 text-xs whitespace-nowrap">{entry.verify_date || '—'}</td>
                    <td className="px-3 py-2 text-xs">{entry.verifier}</td>
                    <td className="px-3 py-2 text-xs">{entry.designation}</td>
                    <td className="px-3 py-2 text-xs">
                      <div className="flex gap-1 items-center">
                        <Link
                          href={`/ghoshnapatra/${entry.id}`}
